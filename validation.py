...
def ml_analysis(real_df, synthetic_df, data_manager, args, task=None, split=0.8):
    ...
    tr_real_x = tr_real.drop(columns=['cardio'])
    tr_real_y = tr_real['cardio']
    te_real_x = te_real.drop(columns=['cardio'])
    te_real_y = te_real['cardio']
    tr_syn_x = tr_syn.drop(columns=['cardio'])
    tr_syn_y = tr_syn['cardio']
